...
power_analysis<-function(alter.prob=c(1/2,1/3,1/6),null.prob=0.05,
                         ...
                         null.level=0.95,alter.power=0.8,initial.sample.size=1000,rep.num=1000,thres=0.01){
...
power_analysis(initial.sample.size = 20,rep.num=1000,insit.trials=c(200,3000,1000),alter.prob = c(0.05,0.05,0.06))
...
power_analysis_plot<-function(alter.prob=c(0.9,0.9,0.95),null.prob=0.9,
                         ...
                         null.level=0.95,alter.power=0.8,initial.sample.size=100,rep.num=1000,thres=0.02){
  ...
  if(alter.power-power>thres) {
    ...
    if((alter.power-power>thres) & (alter.power-record.sample.size[i-1,2]>thres)) {
      ...
      return(ggplot(data = record.sample.size,aes(x=sample.size,y=power))+geom_line()+geom_point()+theme_bw()
...
power_analysis_plot(alter.power = 0.9,thres = 0.02,alter.prob = c(0.05,0.05,0.06),initial.sample.size = 100)
power_analysis(alter.power = 0.9,thres = 0.01,alter.prob = c(0.05,0.05,0.055),initial.sample.size = 5)

## test
power_analysis_plot(thres = 0.025,rep.num = 2000,initial.sample.size = 500)
